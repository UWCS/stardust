...
Sponsor images are located next to their `.md` file **with the same name** -- `.png` / `.svg` only instead of `.md`. Note when adding new images, these should be transparent around the logo -- both so light/dark mode looks good, and so the silhouette effect works properly.
